<form x-data="{ loading: false, coordinates: @entangle('coordinates'), city: false}"
    ...
    wire:submit="save">
    <p class="text-sm text-gray-600 dark:text-gray-400">To use the weather module you must give your permission to locate your position.
    </p>
    <div class="w-full">
        <div for="geolocation" @click="loading = true; city = false; getLocation((coords) => coordinates = coords)"
    ...
    <template x-if="coordinates !== false && city !== false">
        <div x-cloak x-show="coordinates !== false"
            class="flex items-center justify-between shadow p-4 mt-6 text-sm bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div class="space-x-2 flex flex-row items-center">
                <x-mdi-map-marker-radius class="w-8 text-gray-600 dark:text-gray-400" />
                <div>
