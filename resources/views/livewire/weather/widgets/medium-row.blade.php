<div class="medium-row bg-white dark:bg-gray-800 shadow-sm rounded-lg sm:px-6 lg:px-8 flex flex-col items-center justify-center py-2 relative">
<p class="absolute top-1 left-1 mt-1 ml-2 text-xs text-gray-300 dark:text-gray-600">{{ $location->getName() }}</p>
    ...
    <div class="flex flex-wrap overflow-y-auto justify-center items-center">
        ...
        @if($location->airPollution())
            <x-weather.condition class="relative group cursor-pointer hover:bg-gray-100/60 dark:hover:bg-gray-900/40 ">
                ...
                <p>{{__('Air quality: ')}}</p>
                <p class="font-semibold @airPollutionTextColor($location->airPollution()->getAirQualityIndexDescription())">
