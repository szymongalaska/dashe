<div
    class="basis-full min-h-80 sm:px-6 lg:px-8 p-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    <header>
    ...
    <div class="max-h-64 overflow-y-auto border border-gray-300 bg-gray-100/20 dark:bg-gray-900/80 dark:border-gray-700 py-2 px-4 rounded-lg mt-2 shadow-md">
        <ul>
            @if(empty($locations))
            ...
            @foreach($locations as $key => $location)
                <li class="text-sm py-2">
                    @if($loop->first)
                        <span class="inline-flex gap-2 dark:text-gray-200"><x-mdi-near-me class="w-4" />{{$location['name']}},
