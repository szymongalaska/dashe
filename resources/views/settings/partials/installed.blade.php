<div class="bg-white p-4 sm:p-8 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg space-y-6">
    <section>
        ...
        <form method="post" action="{{ route('settings.uninstall') }}" x-data="{ checkedItems: [] }">
            @csrf
            @method('delete')
            <div class="border rounded-md my-2">
                @foreach($modules as $module)
                    <div x-data=""
                        class="flex items-center p-2 hover:bg-gray-100 dark:hover:bg-gray-900 rounded-lg cursor-pointer"
                        @click="$el.querySelector('input').click()">
                        <input id="{{$module->name}}" type="checkbox" name="modules[]" value="{{$module->id}}"
                            x-model="checkedItems"
                            class="w-4 h-4 checked:bg-purple-900 dark:checked:bg-purple-900 bg-gray-100 border-gray-300 rounded-sm focus:ring-purple-700 dark:focus:ring-purple-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            ...
            <x-danger-button class="mt-6" x-data="" x-bind:disabled="checkedItems.length === 0"
