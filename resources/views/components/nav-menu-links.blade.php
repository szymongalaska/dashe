<div class="flex justify-between h-16 flex-col min-h-screen">
    <div class="flex justify-center pt-8 pb-4 border-gray-300 border-b">
        <x-mdi-monitor-dashboard class="block h-9 w-auto fill-current text-gray-200" />
    </div>

    <div class="space-y-8 sm:-my-px flex flex-col">
        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
            <x-mdi-home class="block h-9 w-auto fill-current dark:text-gray-200" />
        </x-nav-link>
        ...
        <x-nav-link :href="route('settings.index')" :active="request()->routeIs('settings.index')">
            <x-mdi-cogs class="block h-9 w-auto fill-current dark:text-gray-200" />
        </x-nav-link>
    </div>
